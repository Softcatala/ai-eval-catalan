...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("benchmarks/parlament_parla_v3_test_400")
    )
    ...
    metadata_url = hf_hub_url(DATASET, "corpus/files/clean_test_parlament_short.csv", repo_type=REPO_TYPE, revision=REVISION)
    with requests.get(metadata_url, stream=True) as response:
        response.raise_for_status()
        rows = list(csv.DictReader((line.decode("utf-8") for line in response.iter_lines())))
    wanted = {row["identifier"]: row for row in rows[:args.num_samples]}
    records = []
    for shard in range(1, 3):
        archive_url = hf_hub_url(DATASET, f"corpus/speech/{SPLIT}/{SPLIT}_{shard}.tar.gz", repo_type=REPO_TYPE, revision=REVISION)
        with requests.get(archive_url, stream=True) as response:
            ...
            with tarfile.open(fileobj=response.raw, mode="r|gz") as archive:
                for member in archive:
                    ...
                    with archive.extractfile(member) as source, (output / filename).open("wb") as target:
                        shutil.copyfileobj(source, target)
                    duration = sf.info(output / filename).duration
                    if duration <= args.max_duration:
                        row = wanted.pop(identifier)
                        records.append({"id": identifier, "audio": filename, "duration_s": round(duration, 6), "reference": row["text"]})
